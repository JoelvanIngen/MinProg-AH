...
_direction_change_mapping_1 = {
    definitions.RIGHT: definitions.UP,
    definitions.UP: definitions.LEFT,
    definitions.LEFT: definitions.DOWN,
    definitions.DOWN: definitions.RIGHT,
}

_direction_change_mapping_2 = {
    definitions.RIGHT: definitions.DOWN,
    definitions.DOWN: definitions.LEFT,
    definitions.LEFT: definitions.UP,
    definitions.UP: definitions.RIGHT,
}

_direction_change_cascades = {
    (definitions.RIGHT, definitions.UP): _direction_change_mapping_1,
    (definitions.UP, definitions.LEFT): _direction_change_mapping_1,
    (definitions.LEFT, definitions.DOWN): _direction_change_mapping_1,
    (definitions.DOWN, definitions.RIGHT): _direction_change_mapping_1,
    (definitions.RIGHT, definitions.DOWN): _direction_change_mapping_2,
    (definitions.DOWN, definitions.LEFT): _direction_change_mapping_2,
    (definitions.LEFT, definitions.UP): _direction_change_mapping_2,
    (definitions.UP, definitions.RIGHT): _direction_change_mapping_2,
}
...
class Node:
    ...
    def change_direction(self, direction: int, ignore_pos_set: bool = False):
        ...
        if self.next:
            self.next.cascade_position(delta_pos, direction, 1)
    ...
    def cascade_position(self, delta_pos: Vec3D, direction, depth):
        """
        ...
        # v = Vec3D(1, 0, 2)
        # print(f'{repr(v)} x 5 = {v * 5}')
        # print(f'- (depth={depth}')
        # print(self.pos)

        # print(f'{repr(self.pos)} + {delta_pos} * {depth} -> {repr(new_pos)}')
        self.pos += delta_pos * depth
        # self.protein.order[self.id] = direction  # Obviously bugged!

        # print(self.pos)

        if self.next is not None:
            self.next.cascade_position(delta_pos, direction, depth + 1)
