...
from utils import index_sequence, collate_fn_order
from dataset_order import FoldDatasetOrder
...
def train_loop(batch_size, norm_val, dataloader, model, loss_fn, optimizer):
	...
	# loop over dataset
	loss_total = 0
	for (sequences, orders) in dataloader:

		# loop over batch and sum total loss
		#import pdb; pdb.set_trace()
		outputs = model(sequences)
		predicted_orders = outputs.view(-1, model.n_classes)
		orders_ = orders.view(-1)
		loss = loss_fn(predicted_orders, orders_)
		loss_total += loss.float()
...
def main():
	# settings
	model_path = './models/first_test/model_2d_50k_class.pt' # where to save weights
	...
	dataset_full = FoldDatasetOrder('./data/50k_datapoints.csv', n_dimensions = 2)
	dataset = Subset(
			dataset_full,
			range(n_datapoints)
		)
	...
	train_dataloader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=collate_fn_order)
	test_dataloader = DataLoader(test_dataset, batch_size=batch_size, collate_fn=collate_fn_order)
	print("Datasets loaded")

	# instantiate model, loss function and optimizer
	model = AmadeusFoldClassifier(n_molecules = 4) # for padding
	loss_fn = torch.nn.CrossEntropyLoss() # FoldLoss()
