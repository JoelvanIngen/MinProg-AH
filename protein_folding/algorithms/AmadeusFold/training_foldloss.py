...
from utils import index_sequence, collate_fn_with_score
from dataset import FoldDataset
from LSTM_model import AmadeusFold
from custom_loss import FoldLossVectorised
...
def train_loop(batch_size, norm_val, dataloader, model, loss_fn, optimizer):
	...
	# loop over dataset
	loss_total = 0
	for (sequences, coordinates, scores) in dataloader:
...
def main():
	# settings
	model_path = './models/first_test/model_2d_5k_foldloss.pt' # where to save weights
	...
	train_dataloader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=collate_fn_with_score)
	test_dataloader = DataLoader(test_dataset, batch_size=batch_size, collate_fn=collate_fn_with_score)
	print("Datasets loaded")

	# instantiate model, loss function and optimizer
	model = AmadeusFold(n_molecules = 4)
	loss_fn = FoldLossVectorised()
